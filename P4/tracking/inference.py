...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #print("noisyDistance: ", noisyDistance, " emissionModel: ", emissionModel, " pacmanPosition: ", pacmanPosition)
        ...
        if noisyDistance == None: 
            ...
            for p in self.legalPositions:
                ...
                #print("p: ", p, " prob: ", prob)
                allPossible[p] = self.beliefs[p] * prob
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions: 
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                #print("oldPos: ", oldPos, " newPos: ", newPos, " prob: ", prob)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #particles = [None] * self.numParticles
        self.particles = []
        i = 0
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        if noisyDistance == None: #ghost captured
            ...
            for p in self.legalPositions:
                d = util.manhattanDistance(pacmanPosition, p)
                prob = emissionModel[d]
                new_beliefs[p] = old_beliefs[p] * prob
            new_beliefs.normalize()
            if new_beliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
            else: 
                new_particles = util.nSample(new_beliefs.values(), new_beliefs.keys(), self.numParticles)
    ...
    def observeState(self, gameState):
        ...
        if anyTrue:
            ...
            for i in range(0, self.numParticles):
                temp.append(util.sample(positions))
            self.particles = temp
        else:
            self.initializeParticles()
            for i in range(self.numGhosts):
